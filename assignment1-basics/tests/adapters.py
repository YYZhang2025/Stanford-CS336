...
def run_rope(
    ...
    rope.rotation_matrix_table = rope.generate_rotation_matrix(theta, d_k, max_seq_len)
...
def run_transformer_block(
    ...
    block.mha.q_proj.weight.data.copy_(weights["attn.q_proj.weight"])
    block.mha.k_proj.weight.data.copy_(weights["attn.k_proj.weight"])
    block.mha.v_proj.weight.data.copy_(weights["attn.v_proj.weight"])
    block.mha.o_proj.weight.data.copy_(weights["attn.output_proj.weight"])
    ...
    return block(in_features)  # type: ignore[return-value]
...
def run_transformer_lm(
    ...
    for layer_idx in range(num_layers):
        ...
        transformer.layers[layer_idx].mha.o_proj.weight.data.copy_(
    ...
    transformer.output_embeddings.weight.data.copy_(weights["lm_head.weight"])
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    return softmax(
        in_features, dim=dim, dtype=torch.float32
...
def run_gradient_clipping(
    ...
    clip_gradient(parameters, max_l2_norm)  # type: ignore[call-arg]
...
def run_save_checkpoint(
    ...
    save_checkpoint(model, optimizer, iteration, out)  # type: ignore[call-arg]
...
def run_load_checkpoint(
    ...
    return load_checkpoint(src, model, optimizer)  # type: ignore[return-value]
