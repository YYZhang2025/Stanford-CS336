    ...
    while True:
        for data in dataloader:
            ...
            with ctx:
                ...
                loss, info = sft_microbatch_train_step(log_prob, response_mask, model, tokenizer)
...
class SFTDataset(Dataset):
    def __init__(self, prompt_answer_list: List[dict], tokenizer):
        self.data = prompt_answer_list
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        prompt = data["prompt"]
        answer = data["answer"]
        input_ids, labels, response_mask = tokenize_prompt_and_output(prompt, answer, self.tokenizer)
...
def main(
    *,
    ...
    tokenizer = AutoTokenizer.from_pretrained(train_config.model_name)
    all_datasets = load_json_to_list(train_config.data_path)

    os.makedirs(train_config.ckpt_dir, exist_ok=True)

    for num_samples in [128, 256, 512, 1024, "all"]:
        train_config.num_example = num_samples if num_samples != "all" else len(all_datasets)
        train_dataset = all_datasets[:num_samples] if num_samples != "all" else all_datasets

        train_dataset = SFTDataset(train_dataset, tokenizer)
