...
if __name__ == "__main__":
    q, k, v = torch.randn(8, 10, 128, 64), torch.randn(8, 10, 128, 64), torch.randn(8, 10, 128, 64)
    ...
    out_impl = FlashAttention.apply(q, k, v, True)
    out_rel = torch.nn.functional.scaled_dot_product_attention(q, k, v, dropout_p=0.0, is_causal=True)

    loss_impl = out_impl.sum()
    loss_rel = out_rel.sum()
    ...
    # Clear grads before computing reference backward
    q.grad = None
    k.grad = None
    v.grad = None
    ...
    rich.print("[red]Test Forward pass:[/red]")
    assert torch.allclose(out_impl, out_rel, atol=1e-5, rtol=1e-5)
    rich.print("[green] Forward pass test passed![/green]")

    rich.print("[red]Test Backward pass:[/red]")
    assert torch.allclose(q_grad_impl, q.grad, atol=1e-3, rtol=1e-3)
    assert torch.allclose(k_grad_impl, k.grad, atol=1e-3, rtol=1e-3)
    assert torch.allclose(v_grad_impl, v.grad, atol=1e-3, rtol=1e-3)
