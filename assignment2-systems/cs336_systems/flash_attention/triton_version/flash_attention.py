...
def get_flash_attention_triton(version: int = 3):
    if version == 1:
        from .forward_backward_version1 import _flash_backward_triton, _flash_forward_triton
    elif version == 2:
        from .forward_backward_version2 import _flash_backward_triton, _flash_forward_triton
    elif version == 3:
        from .forward_backward_version3 import _flash_backward_triton, _flash_forward_triton
    else:
        raise ValueError(f"Unsupported Triton FlashAttention version: {version}")
    ...
    class FlashAttention(torch.autograd.Function):
        ...
        def backward(ctx, grad_O: torch.Tensor):
            ...
            dQ, dK, dV = _flash_backward_triton(q, k, v, O, L, dO, is_causal=is_causal)
